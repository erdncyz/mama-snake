...
extension GameScene {
    ...
    var webSegments: [SKSpriteNode] {
        get { return objc_getAssociatedObject(self, &AssociatedKeys.webSegments) as? [SKSpriteNode] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.webSegments, newValue, .OBJC_ASSOCIATION_RETAIN) }
    }
    
    var webJoints: [SKPhysicsJoint] {
        get { return objc_getAssociatedObject(self, &AssociatedKeys.webJoints) as? [SKPhysicsJoint] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.webJoints, newValue, .OBJC_ASSOCIATION_RETAIN) }
    }
    
    var lastWebPoint: CGPoint? {
        get { return objc_getAssociatedObject(self, &AssociatedKeys.lastWebPoint) as? CGPoint }
        set { objc_setAssociatedObject(self, &AssociatedKeys.lastWebPoint, newValue, .OBJC_ASSOCIATION_RETAIN) }
    }
    
    var webAnchorPoints: [CGPoint] {
        get { return objc_getAssociatedObject(self, &AssociatedKeys.webAnchorPoints) as? [CGPoint] ?? [] }
        set { objc_setAssociatedObject(self, &AssociatedKeys.webAnchorPoints, newValue, .OBJC_ASSOCIATION_RETAIN) }
    }
    ...
    func createWebStrand(from startPoint: CGPoint, to endPoint: CGPoint) {
        ...
        for i in 0..<segmentCount {
            ...
            let sequence = SKAction.sequence([fadeDelay, fadeOut, remove, SKAction.removeFromParent()])
